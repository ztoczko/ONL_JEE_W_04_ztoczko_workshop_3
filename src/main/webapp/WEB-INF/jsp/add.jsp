                        ...
                        <form class="form-group" action="${"add".concat("?fromPage=".concat(param.fromPage)).concat(param.search == null || param.search.trim().isEmpty() ? "" : "&search=".concat(param.search))}" method="post">
                            <table class="table-hover table bg-white">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input style="border-width: 0; width: 100%; ${(!empty error && error.contains("name")) ? "background-color: lightsalmon" : ""}" value="<c:out value="${name}" default=""/>" type="text" name="name">
                                ...
                                <tr>
                                    <th>e-mail</th>
                                    <td>
                                        <input style="border-width: 0; width: 100%; ${(!empty error && error.contains("email")) ? "background-color: lightsalmon" : ""}" type="email" name="email" value="<c:out value="${email}" default=""/>">
                                ...
                                <tr>
                                    <th>hasło</th>
                                    <td><input style="border-width: 0; width: 100%; ${(!empty error && error.contains("password")) ? "background-color: lightsalmon" : ""}" type="password" name="password1" value="<c:out value="${password1}" default=""/>">
                                ...
                                <tr>
                                    <th>Powtórz hasło</th>
                                    <td><input style="border-width: 0; width: 100%; ${(!empty error && error.contains("password")) ? "background-color: lightsalmon" : ""}" type="password" name="password2" value="<c:out value="${password2}" default=""/>">
                                        <c:if test="${!empty error && error.contains(\"passwordsDontMatch\")}">
                                            <span style="position: relative; text-align: justify-all; width: 100%"> Wprowadzone hasła nie są identyczne</span>
                                        </c:if>
                                    </td>
